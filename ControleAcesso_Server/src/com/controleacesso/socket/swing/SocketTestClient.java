...
public class SocketTestClient extends JPanel {
	...
	public SocketTestClient(final JFrame parent) {
		...
		toPanel.setBorder(BorderFactory.createTitledBorder(new EtchedBorder(),
				"Connect To"));
		...
		sendPanel.setBorder(new CompoundBorder(BorderFactory.createEmptyBorder(
				0, 0, 0, 3), BorderFactory.createTitledBorder("Send")));
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						JOptionPane.showMessageDialog(SocketTestClient.this, ""
								+ ioe.getMessage(), "Error saving to file..",
		...
		CompoundBorder cb = new CompoundBorder(BorderFactory.createEmptyBorder(
				5, 10, 10, 10), connectedBorder);
	...
	private void connect() {
		...
		if (!Util.checkHost(ip)) {
			JOptionPane.showMessageDialog(SocketTestClient.this,
					"Bad IP Address", "Error connecting",
					JOptionPane.ERROR_MESSAGE);
			ipField.requestFocus();
			ipField.selectAll();
		...
		try {
			if (isSecure == false) {
				...
				System.out.println("Connectig in secure mode : " + ip + ":"
						+ portNo);
				// SocketFactory factory = SSLSocketFactory.getDefault();

				TrustManager[] tm = new TrustManager[] { new MyTrustManager(
						SocketTestClient.this) };
	...
	public void error(String error) {
		if (error == null || error.equals(""))
			return;
		JOptionPane.showMessageDialog(SocketTestClient.this, error, "Error",
				JOptionPane.ERROR_MESSAGE);
	}
	...
	public void error(String error, String heading) {
		if (error == null || error.equals(""))
			return;
		JOptionPane.showMessageDialog(SocketTestClient.this, error, heading,
				JOptionPane.ERROR_MESSAGE);
	}
