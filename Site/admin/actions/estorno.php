...
if (acessoPermitido($mainConnection, $_SESSION['admin'], 250, true)) {
    ...
    if ($_POST['pedido'] != '' and isset($_POST['pedido'])) {
        ...
        if ($pedido["IN_TRANSACAO_PDV"] == 0) {
            // VENDAS PELO PDV NÃO SÃO ESTORNADAS DO BRASPAG
            try {
        ...
        if ($descricao_erro == '') {
            ...
            if (($response->CorrelationId == $ri) OR ($pedido["IN_TRANSACAO_PDV"] == 1)) {

                if (($response->TransactionDataCollection->TransactionDataResponse->Status == '0') OR ($pedido["IN_TRANSACAO_PDV"] == 1)) {
