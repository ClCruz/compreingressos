...
$codigoSeguranca = fgets($fh); //Busca o código do segurança e gera a hash com o algoritmo MD5.
...
if (isset($_COOKIE['pedido'])) {
	$test = executeSQL($mainConnection, 'SELECT 1 FROM MW_PEDIDO_VENDA WHERE ID_CLIENTE = ? AND ID_PEDIDO_VENDA = ?', array($_SESSION['user'], $_COOKIE['pedido']), true);
	if (!$test[0]) header("Location: pagamento_cancelado.php");
	
	executeSQL($mainConnection, 'DELETE FROM MW_ITEM_PEDIDO_VENDA WHERE ID_PEDIDO_VENDA = ?', array($_COOKIE['pedido']));
	executeSQL($mainConnection, 'DELETE FROM MW_PEDIDO_VENDA WHERE ID_PEDIDO_VENDA = ?', array($_COOKIE['pedido']));
	$newMaxId = $_COOKIE['pedido'];
} else {
