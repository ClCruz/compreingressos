...
if (isset($_GET['evento']) and is_numeric($_GET['evento'])) {
	...
	if (!empty($rs)) {
		...
		if (!empty($rs)) {
			...
			$query = "SELECT E.DS_EVENTO, MIN(A.ID_APRESENTACAO) ID_APRESENTACAO, CONVERT(VARCHAR(10), A.DT_APRESENTACAO, 103) DT_APRESENTACAO,
				    ...
				    GROUP BY E.DS_EVENTO, CONVERT(VARCHAR(10), A.DT_APRESENTACAO, 103),
					    A.HR_APRESENTACAO, A.DT_APRESENTACAO,
					    DATEDIFF(HH, DATEADD(HH, ?, CONVERT(DATETIME, CONVERT(VARCHAR, A.DT_APRESENTACAO, 112) + ' '
					    + LEFT(HR_APRESENTACAO,2) + ':' + RIGHT(HR_APRESENTACAO,2) + ':00')) ,GETDATE() )
				    ORDER BY DT_APRESENTACAO_ORDER, HR_APRESENTACAO";
			
			$params = array($rs['QT_HR_ANTECED'], $_GET['evento'], $rs['QT_HR_ANTECED']);
