...
if ($parametros['valor_total'] > 0) {
	...
	if (isset($retorno['codigo_erro'])) {
		...
		ob_end_clean();
		header("Location: pagamento_cancelado.php?falha".(isset($_GET['tag']) ? $falha['tag_voltar'] : ''));
		exit();
	} else {
		ob_end_clean();
		header("Location: pagamento_ok.php?pedido=".$retorno['codigo_pedido'].(isset($_GET['tag']) ? $campanha['tag_avancar'] : ''));
		exit();
	}
