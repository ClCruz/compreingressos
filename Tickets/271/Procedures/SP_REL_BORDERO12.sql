...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SP_REL_BORDERO12]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [DBO].[SP_REL_BORDERO12]
GO

/* EQUIVALENTE AO ;9 (SOMENTE PARA TODOS OS SETORES) */
CREATE PROCEDURE [dbo].[SP_REL_BORDERO12]
	@DtIniApr varchar(8), 
	@DtFimApr varchar(8), 
	@codPeca int = null,
