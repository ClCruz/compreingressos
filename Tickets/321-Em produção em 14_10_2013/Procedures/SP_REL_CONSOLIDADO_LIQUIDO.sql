...
SELECT DISTINCT CODTIPDEBBORDERO, CODTIPBILHETEDEB, COUNT(1) AS QTDE
...
SELECT distinct
	...
	,HORSESSAO
	,DBO.GETQTDEEXCEDENTES(DATAPRESENTACAO, HORSESSAO, QTDLIMITEINGRPARAVENDA, T1.QTDE) AS QTDE
	,(DBO.GETQTDEEXCEDENTES(DATAPRESENTACAO, HORSESSAO, QTDLIMITEINGRPARAVENDA, T1.QTDE) * VALINGRESSOEXCEDENTE) * - 1 AS PAGTO
	,DEBBORDERO COLLATE Latin1_General_CI_AS AS CANAL_VENDA
	,'Descontos' AS OPERACAO
	,3 AS ORDEM
	,#TMP_RESUMO.CODTIPBILHETEDEB
	,1 AS CONVITE_EXCEDENTE
	,0 AS QTDE_REAL
	,0 AS PAGTO_REAL
	,0 AS LUGARES
FROM #TMP_RESUMO
INNER JOIN #TMP_EXECEDENTES T1 ON T1.CODTIPBILHETEDEB = #TMP_RESUMO.CODTIPBILHETEDEB
	AND T1.CODTIPDEBBORDERO = #TMP_RESUMO.CODTIPDEBBORDERO
WHERE ISNULL(#TMP_RESUMO.CODTIPBILHETEDEB, 0) <> 0
GROUP BY CONVERT(VARCHAR, DATAPRESENTACAO, 112)
	,DATAPRESENTACAO
	,HORSESSAO
	,DEBBORDERO
...
SELECT DISTINCT CONVERT(VARCHAR, DATAPRESENTACAO, 112) AS DATA_APRESENTACAO
	...
	,0 AS QTDE_REAL
	,0 AS PAGTO_REAL
	,DBO.GETLUGARES(@CODPECA, DATAPRESENTACAO, DATAPRESENTACAO, CASE WHEN @CODSALA = -1 THEN NULL ELSE HORSESSAO END, CASE WHEN @CODSALA = -1 THEN NULL ELSE CODSALA END) AS LUGARES
FROM #TMP_RESUMO
