...
!		 !			   !			!				  !	tado em PHP.											  !	
+========+=============+============+=================+===========================================================+'
!   2    !     #355    ! 25/10/2013 ! Edicarlos S. B. ! No calculo de % adicionado select p/ tabApresentacao	  !
+=================================================================================================================+
*/
...
SELECT tabTipDebBordero.CodTipDebBordero
	...
	,(Select count(1)
		from tablancamento l
		where codapresentacao = @CodApresentacao
		and codtipbilhete = tabTipDebBordero.CodTipBilhete
		and codtiplancamento = 1
		and not exists (select 1 from tablancamento l2
							where codapresentacao = @CodApresentacao
							and codtipbilhete = tabTipDebBordero.CodTipBilhete
							and codtiplancamento = 2
							and l.indice = l2.indice
							and l.numlancamento = l2.numlancamento)) as IngressosExcedentes
INTO #TEMP1
FROM tabDebBordero
...
SELECT CodTipDebBordero
	...
	,CASE 
		...
		ELSE DBO.GETINGEXCEDENTES(CodTipBilhete, Valor, CASE
															WHEN TipValor = 'P' 
																THEN ((SELECT ValPeca FROM tabApresentacao WHERE CodApresentacao = @CodApresentacao) * ValIngressoExcedente) / 100
																ELSE ValIngressoExcedente
															END, QtdLimiteIngrParaVenda, IngressosExcedentes)
		END AS Valor
	,TipValor
	,VlMinimoDebBordero
