		...
		if (!empty($rs)) {
			$query = "WITH RESULTADO AS (
						SELECT E.DS_EVENTO, A.ID_APRESENTACAO, CONVERT(VARCHAR(10), A.DT_APRESENTACAO, 103) DT_APRESENTACAO,
								...
								(SELECT ISNULL(SUM(1), 0) FROM ".$nomeBase."..TABSALDETALHE D
										INNER JOIN ".$nomeBase."..TABAPRESENTACAO A2 ON A2.CODSALA = D.CODSALA
										WHERE D.TIPOBJETO = 'C' AND A2.CODAPRESENTACAO = A.CODAPRESENTACAO
										AND NOT EXISTS (SELECT 1 FROM ".$nomeBase."..TABLUGSALA L
					...
					FROM RESULTADO R1
					WHERE DISPONIVEIS = (SELECT MAX(DISPONIVEIS) FROM RESULTADO R2 WHERE R1.DT_APRESENTACAO = R2.DT_APRESENTACAO AND R1.HR_APRESENTACAO = R2.HR_APRESENTACAO)
					GROUP BY DS_EVENTO, DT_APRESENTACAO, HR_APRESENTACAO, DT_APRESENTACAO_ORDER, TELEFONE, DISPONIVEIS
