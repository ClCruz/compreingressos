	...
	if (!empty($rs)) {
		...
		if (!empty($rs)) {
			$query = "WITH RESULTADO AS (
					...
					FROM RESULTADO R1
					WHERE DS_PISO = (SELECT TOP 1 DS_PISO FROM RESULTADO R2 WHERE R1.DT_APRESENTACAO = R2.DT_APRESENTACAO AND R1.HR_APRESENTACAO = R2.HR_APRESENTACAO ORDER BY DS_PISO)
