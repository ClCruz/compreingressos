...
// tem bilhete itau sem bin?
$query = "SELECT E.ID_BASE, E.CODPECA, AB.CODTIPBILHETE FROM MW_RESERVA R
			...
			WHERE R.ID_SESSION = ? AND R.CD_BINITAU IS NULL
			GROUP BY E.ID_BASE, E.CODPECA, AB.CODTIPBILHETE";
$result = executeSQL($mainConnection, $query, array(session_id()));

while ($rs = fetchResult($result)) {
	$conn = getConnection($rs['ID_BASE']);
	$query = "SELECT 1 FROM TABPECA WHERE CODPECA = ? AND CODTIPBILHETEBIN = ?";
	$rs = executeSQL($conn, $query, array($rs['CODPECA'], $rs['CODTIPBILHETE']), true);
...
$query = "SELECT E.ID_BASE, E.CODPECA, AB.CODTIPBILHETE FROM MW_RESERVA R
			...
			INNER JOIN MW_EVENTO E ON E.ID_EVENTO = A.ID_EVENTO
			INNER JOIN MW_APRESENTACAO_BILHETE AB ON AB.ID_APRESENTACAO = A.ID_APRESENTACAO AND AB.IN_ATIVO = 1 AND AB.ID_APRESENTACAO_BILHETE = R.ID_APRESENTACAO_BILHETE
			WHERE R.ID_SESSION = ? AND R.NR_BENEFICIO IS NULL
			GROUP BY E.ID_BASE, E.CODPECA, AB.CODTIPBILHETE";
$result = executeSQL($mainConnection, $query, array(session_id()));

while ($rs = fetchResult($result)) {
	$conn = getConnection($rs['ID_BASE']);
	$query = "SELECT 1 FROM TABPROMOCAOPECA WHERE CODPECA = ? AND CODTIPBILHETE = ?";
	$rs = executeSQL($conn, $query, array($rs['CODPECA'], $rs['CODTIPBILHETE']), true);
