...
if (acessoPermitido($mainConnection, $_SESSION['admin'], 384, true)) {
    ...
    if ($_GET['action'] == 'busca' and isset($_GET['cboPeca']) and isset($_GET['cboPromocao'])) {
        ...
        $_GET['cboPromocao'] = explode('|', $_GET['cboPromocao']);
        $_GET['cboBilhete'] = $_GET['cboPromocao'][1];
        $_GET['cboPromocao'] = $_GET['cboPromocao'][0];
        ...
        $_POST['cboPromocao'] = explode('|', $_POST['cboPromocao']);
        $_POST['cboBilhete'] = $_POST['cboPromocao'][1];
        $_POST['cboPromocao'] = $_POST['cboPromocao'][0];
        ...
        $query = "SELECT DISTINCT T.CODTIPPROMOCAO, T.NOMPROMOCAO, P.CODTIPBILHETE, B.TIPBILHETE
                    FROM CI_MIDDLEWAY..MW_EVENTO E
                    INNER JOIN CI_MIDDLEWAY..MW_ACESSO_CONCEDIDO A ON A.CODPECA = E.CODPECA AND A.ID_BASE = E.ID_BASE
                    INNER JOIN TABPROMOCAOPECA P ON P.CODPECA = A.CODPECA
                    INNER JOIN TABTIPPROMOCAO T ON T.CODTIPPROMOCAO = P.CODTIPPROMOCAO
                    INNER JOIN TABTIPBILHETE B ON B.CODTIPBILHETE = P.CODTIPBILHETE
        ...
        while($rs = fetchResult($result)){
            $combo .= '<option value="'. $rs["CODTIPPROMOCAO"].'|'.$rs["CODTIPBILHETE"] .'"' . (($_GET['cboPromocao'] == $rs['CODTIPPROMOCAO']) ? ' selected' : '') . '>'. utf8_encode($rs["NOMPROMOCAO"].' - '.$rs["TIPBILHETE"]) .'</option>';
            if ($_GET['excel'] and $_GET['cboPromocao'] == $rs["CODTIPPROMOCAO"].'|'.$rs["CODTIPBILHETE"]) {
                $text = utf8_encode($rs["NOMPROMOCAO"].' - '.$rs["TIPBILHETE"]);
                break;
            }
        }
        ...
        $file_name = preg_replace('/[^\d]/', '', microtime()).'.csv';

        if (!move_uploaded_file($_FILES['csv']['tmp_name'], './temp/'.$file_name)) {
            echo "false?erro=Falha ao copiar o arquivo.";
            die();
        }

        $path = "\\\\".$_SERVER['LOCAL_ADDR']."\\csv\\".$file_name;

        $query = 'EXEC prc_importa_codigos_promocionais ?,?,?,?';
        $params = array($path, $_POST['cboPeca'], $_POST['cboPromocao'], $_POST['txtDescricao']);
